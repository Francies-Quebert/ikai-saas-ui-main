...
const InventoryManufacturerWiseRevenue = (props) => {
  ...
  useEffect(() => {
    let tmpDataSource = [[`Manufacturer`, "Amount"]];

    if (dataSource && dataSource.length > 0) {
      //   console.log("hari", dataSource);
      dataSource.map((ii) => {
        return tmpDataSource.push([ii.MfrDesc, parseFloat(ii.Amount)]);
      });
    }
  ...
  return (
    ...
    >
      ...
      >
        <Row>
          <Col flex="1 1 200px">Manufacturer Wise Revenue</Col>
          <Col flex="0 1 30px">
...
export default InventoryManufacturerWiseRevenue;
