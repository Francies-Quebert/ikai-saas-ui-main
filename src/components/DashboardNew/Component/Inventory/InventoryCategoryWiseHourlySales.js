...
const InventoryCategoryWiseHourlySales = (props) => {
  ...
  useEffect(() => {
    if (dataSource && dataSource.length > 0) {
      ...
      dataSource.forEach((row) => {
        if (!distinctCategory.find((distinct) => distinct === row.CatDesc)) {
          distinctCategory.push(row.CatDesc);
        }
      });
      ...
      // console.log("ssssssss", distinctCategory, distinctBillHour);

      let tmpDataSource = [`Hour`, ...distinctCategory];
      ...
      distinctBillHour.forEach((hour) => {
        ...
        tmpDataSource.forEach((row) => {
          if (row !== "Hour") {
            let sumValue = 0;
            dataSource
              .filter((item) => item.BillHour === hour && item.CatDesc === row)
              .forEach((result) => {
                // console.log("aaaaa", result);
                sumValue += parseFloat(result.Amount);
              });
  ...
  return (
    ...
    >
      ...
      >
        <Row>
          <Col flex="1 1 200px">Hourly Category Wise Revenue</Col>
          <Col flex="0 1 30px">
...
export default InventoryCategoryWiseHourlySales;
